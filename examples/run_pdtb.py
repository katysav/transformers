...
def train(args, train_dataset, model, model_config, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        ...
        output_train_file = os.path.join(
            args.output_dir, "train_results.txt")
                ...
                if args.local_rank in [-1, 0] and args.train_logging_steps > 0 and global_step % args.train_logging_steps == 0:
                    for name, param in model.named_parameters():
                        ...
                        if param.grad is not None:
                            ...
                            tb_writer.add_scalar(
                                name + '_grad_norm', grads_norm, global_step)
                        # else:
                ...
                if args.local_rank in [-1, 0] and args.train_logging_steps > 0 and global_step % args.train_logging_steps == 0:
                    ...
                    for name, param in model.named_parameters():
                        ...
                        if param.grad is not None:
                            ...
                            grads_norm = torch.norm(grads, p=2, dim=0)
                            tb_writer.add_scalar(
                                name + '_clipped_grad_norm', grads_norm, global_step)


                    tb_writer.add_scalar(
                    'total_grad_norm', total_norm, global_step)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        # required=True,
        help="The output directory where tensorboard logs will be written.",
    )
    # Other parameters
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_logging_steps", type=int, default=500, help="Log training progress every X updates steps.")
